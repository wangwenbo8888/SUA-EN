...
//    _pScene3d = NULL;
    ...
    _realTimeShowPath = NULL;      // 实时图像窗口显示声窗
    _realTimeHidePath = NULL;      // 实时图像窗口隐藏声窗

    _realTimeShowFocus = NULL;     // 实时图像窗口显示自然焦点
    _realTimeHideFocus = NULL;     // 实时图像窗口隐藏自然焦点
    ...
    _targetZoomOut = NULL;         // 图像放大
    _targetZoomIn = NULL;          // 图像缩小
    _targetShowDeleted = NULL;     // 显示删除图像
    _targetHideDeleted = NULL;     // 隐藏删除图像

    _targetSelectInvert   = NULL;  // 反选
    _targetSmartSelect    = NULL;  // 自动选择肌瘤内的辐照单元
    _targetResumeDelected = NULL;  // 恢复删除的辐照单元

    _targetShowContours = NULL;    // 显示肌瘤轮廓
    _targetHideContours = NULL;    // 隐藏肌瘤轮廓

    _targetShowDraw = NULL;         // 显示勾画轮廓
    _targetHideDraw = NULL;         // 隐藏勾画轮廓

	_targetDrag = NULL;             //手动编辑轮廓
	
    
    _targetDragConfirm = NULL;      //确定拖动轮廓
    ...
    _layerZoomOut = NULL;
    _layerZoomIn = NULL;
    _layerShowDeleted = NULL;     // 显示删除图像
    _layerHideDeleted = NULL;     // 隐藏删除图像

    _layerSelectInvert   = NULL;  // 反选
    _layerResumeDelected = NULL;  // 恢复删除点
    ...
    _pChoseimage = new Choseimage(this);    // 生成任意旋转成像面窗口
    _pHeaderControl = new HeaderControl(this);    // 生成治疗头控制窗口
...
void NewMainWindow::showTableViewToolTip(const QModelIndex &index)
{
    ...
    {
        QToolTip::showText(QCursor::pos(), index.data().toString() + "瓦");
    }
    else if (index.column()==4)
    {
        QToolTip::showText(QCursor::pos(), index.data().toString() + "个");
    }
    else
    {
        QToolTip::showText(QCursor::pos(), index.data().toString());
...
void NewMainWindow::initReserarchMenu()
{
    ...
    {
        _researchShow = _researchMenu->addAction("显示科研对话框");
...
void NewMainWindow::layerHideElement()     // layer窗口隐藏辐照单元
{
    if (_layerScene!=NULL)
    {
...
void NewMainWindow::layerZoomOut()         // layer窗口放大图像
{
    if (_layerScene!=NULL)
    {
        _layerScene->zoomOut();
...
void NewMainWindow::layerZoomIn()          // layer窗口缩小图像
{
    if (_layerScene!=NULL)
    {
        _layerScene->zoomIn();
...
void NewMainWindow::layerResumeImages()    // layer窗口恢复原始图像大小
{
    if (_layerScene!=NULL)
    {
        _layerViewer2DInfo.scal = 1.0;
...
void NewMainWindow::initIrradiatePlan()
{
    ...
    int size = z.size();
    if (ui->comboBox_part->currentText()=="自主模式")
//    if (ui->comboBox_part->currentText()!="子宫肌")
    {
...
//    {
//        _realTimeChange = _realTimeMenu->addAction("更改计划图像");
...
//        connect(_tableCopy,SIGNAL(triggered(bool)),this,SLOT(copyRows()));
    ...
    {
        _tableDelete = _tableMenu->addAction("删除");
        connect(_tableDelete,SIGNAL(triggered(bool)),this,SLOT(deleteRows()));
    }

//    if (_tableStop==NULL)
//    {
//        _tableStop = _tableMenu->addAction("停止");
...
//    if(_seriesAcquired)
//    {
//        QMessageBox::information(0,"定位中","请等待定位结束后再关闭软件","Confirm",0);
//        return;
//    }
...
void NewMainWindow::init()
{
    ...
    Delay_MSec(5000);
    initMsgCtrl();
    initSchedule();    // 治疗计划系统初始化
    initGrabber();     // 图像采集系统初始化
    Delay_MSec(30000);
    initCore();        // 功放驱动初始化
    initLocation();    // 机械移动系统初始化
    initDate();
...
//    ui->comboBox_part->setCurrentText("子宫肌瘤");
    ui->comboBox_part->addItem(QWidget::tr("Custom Mode"));
    ui->comboBox_part->setCurrentText("自主模式");
...
void NewMainWindow::openPump()    // 开泵
{
//    if (_water!=NULL)
//    {
//        _water->openPump();
...
void NewMainWindow::closePump()    // 关泵
{
//    if (_water!=NULL)
//    {
//        _water->closePump();
...
//            this,SLOT(refreshAmpPara(DataFlag,QByteArray&)));
    ...
    // 设置超声驱动图标可用  TODO要查询超声驱动是否真的可用
    ui->label_hifu_driver_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/超声驱动.png")));
...
void NewMainWindow::addScene(QImage image)
{
    ...
    if (iter==_sceneMap.end()) //已有相同角度则不添加
    {
...
void NewMainWindow::refreshRealtimeImage(QImage image)    // 更新实时图像，方便移动端调用
{
...
//        qDebug()<<"配置文件："<<RESOLUTION<<X_OFFSET<<Y_OFFSET;
        ...
        _subSysStatus[SUPER_B]= true; // 超声状态正常
...
#endif
        ui->label_water_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/水循环.png")));
        ui->label_water_text->setStyleSheet(WORK);
        _subSysStatus[WATER_CIRCLE]= true;    // 水循环系统正常
...
#endif
        ui->label_water_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/水循环-不正常.png")));
...
void NewMainWindow::initLocation() {
    ...
    if (_location->initialize())
    {
        ...
        _subSysStatus[MECHINE_MOVE]= true;    // 机械移动系统正常
    }
    else
    {
...
void NewMainWindow::on_status_updated(DataFlag dFlag)
{
    ...
    case POWER_AMPLIFIERS_NOT_READY:
        ...
        ui->label_hifu_driver_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/超声驱动-不正常.png")));
...
//        Heartthread_thread->start();
    ...
    case POSITION_AVAILABLE:
        ui->label_position_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/机械移动.png")));
        ui->label_position_text->setStyleSheet(WORK);
        break;
    case POSITION_NONAVAILABLE:
        ui->label_position_icon->setPixmap(QPixmap::fromImage(QImage(":/icons/img/底部icon/机械移动-不正常.png")));
...
bool NewMainWindow::deleteFile(QString file)
{
    ...
    {
        if (!fileInfo.dir().remove(file))  // 删除未执行的计划
        {
            qCDebug(NEWMAINWINDOW())<<NEWMAINWINDOW().categoryName()
...
//    _grabber->cutscreen = true;
    if (_realtimeScene!=NULL)
    {
        ...
        if(image.save(fFullpath))
        {
            ...
            msgBox.exec();
//            ui->toolButton_save_image_2->setText(QString("保存成功"));
...
//            });
            ...
            msgBox.setButtonText(QMessageBox::Ok, "OK");
            msgBox.exec();
//            ui->toolButton_save_image_2->setText(QString("保存失败"));
//            QTimer::singleShot(500,[this](){
//                ui->toolButton_save_image_2->setText(QString("Save Image"));
//            });
        }
...
void NewMainWindow::on_toolButton_acquire_image_clicked()
{
    ...
    // 采集图像结束，设置水平滑动条正常，图像选择功能正常，图像张数显示正常
...
// 创建患者对应的计划文件夹，如果文件夹创建时间不超过一小时，
// 则认为是相同的患者，否则删除重新创建
void NewMainWindow::createPlanFold()
{
    deletePlanFold();    // 删除以前保存计划的文件夹
    QDir fold;
...
//        downScene->setTargetDrawable(true);
        ...
        // 以自然焦点所在辐照层面为中心，上下裁剪，向上10mm为上一辐照层面，向下10mm为下一辐照层面
...
void NewMainWindow::realTimeShowPath()  // 显示实时图像上声束
{
    if (_realtimeScene!=NULL)
    {
...
void NewMainWindow::realTimeHidePath()  // 隐藏实时图像上声束
{
    if (_realtimeScene!=NULL)
    {
...
//        connect(_append,SIGNAL(switchChanged(bool)),this,SLOT(switchChanged(bool)));
    ...
    _isIrradiating = false;    // 退出某一点在辐照中状态
    emit setIrradiating(_isIrradiating);
    setLocationControlEnable(true);
    // 设置治疗相关按钮可用
    // setTreatmentControlEnable();
...
void NewMainWindow::synReportInfo()
{
    ...
    outputTreatmentLog("80%",_startTime,_endTime,QString::number(_totalSpot),totalTime,deep,"中惠",pulse);
    emit setTerapyInfo("80%",_startTime,_endTime,QString::number(_totalSpot),totalTime,deep,"中惠",pulse,getDisplayPower(value*value));
...
qDebug()<<"disp size3====="<<_irradiatePlan->getDisplaySize();
    ...
    {
        refreshRemainTime();
        QString str = "剩余时间 " + getTimeString(m_totalTime.remainTime);
        ui->label_time_remain->setText(str);
...
QString NewMainWindow::getTotalTime()
{
    ...
    QString label = "治疗时间 " + getTimeString(total);
    return label;
}
...
QString NewMainWindow::getRemainTotalTime()
{
    ...
    QString label = "剩余时间 " + getTimeString(m_totalTime.remainTime);
    return label;
}
...
void NewMainWindow::on_commandLinkButton_generate_plan_clicked()
{
    ...
    SPOT_DISTRIBUTE dist;    // 此平面治疗点分布情况
    int angle = -90;
...
void NewMainWindow::targetShowElement()    // 显示辐照单元
{
    if (_targetScene!=NULL)
    {
...
void NewMainWindow::targetShowContours()   // target显示肌瘤轮廓
{
    if (_targetScene!=NULL)
    {
        _contoursVisiable = true;
...
void NewMainWindow::targetShowDraw()       // target显示勾画轮廓
{
    QMap<int,DrawScene*>::iterator iter = _targetSceneMap.begin();
    for (; iter != _targetSceneMap.end(); ++iter)
    {
...
void NewMainWindow::targetHideDraw()       // target隐藏勾画轮廓
{
    QMap<int,DrawScene*>::iterator iter = _targetSceneMap.begin();
    for (; iter != _targetSceneMap.end(); ++iter)
    {
...
void NewMainWindow::targetHideContours()   // target隐藏肌瘤轮廓
{
    if (_targetScene!=NULL)
    {
...
void NewMainWindow::targetZoomOut()        // target窗口图像放大
{
    if (_targetScene!=NULL)
    {
...
void NewMainWindow::targetZoomIn()         // target窗口图像缩小
{
    if (_targetScene!=NULL)
    {
        _targetScene->zoomIn();
...
void NewMainWindow::layerShowDeleted()     // layer显示删除的辐照单元
{
    showDeletedElement();
}
...
void NewMainWindow::layerResumeDeleted()   // 恢复删除单元
{
    QMap<int,DrawScene*>::iterator iter = _targetSceneMap.begin();
    for (; iter != _targetSceneMap.end(); ++iter)
    {
...
void NewMainWindow::layerHideDeleted()     // layer隐藏删除的辐照单元
{
    hideDeletedElement();
}

void NewMainWindow::targetShowDeleted()    // target显示删除单元
{
    showDeletedElement();
}
...
void NewMainWindow::targetHideDeleted()    // target隐藏删除单元
{
    hideDeletedElement();
}

void NewMainWindow::targetHideElement()    // 隐藏未标红的辐照单元
{
    if (_targetScene!=NULL)
    {
...
//    if (ui->comboBox_part->currentText()=="子宫肌瘤")
    if (ui->comboBox_part->currentText()=="子宫肌")
    {
        int deep = ui->comboBox_deepth->currentText().toInt();
        Element ele;
...
{
    if (ui->comboBox_irradiate_model->currentText()=="由内到外")
    {
        m_eModel = Model_inner_to_outer;
    }
    else if (ui->comboBox_irradiate_model->currentText()=="由外到内")
    {
...
void NewMainWindow::on_pushButton_pump1control_clicked()
{
    if(!m_bPump1Open)
    {
        if(ui->label_levle_value->text()=="低")
        {
            QMessageBox msgBox("Notice",
                               "水箱当前液位过低，水囊进水功能暂停使用",
                               QMessageBox::Information,
...
void NewMainWindow::on_treatmentTime_updated(){
    ...
    QString str = "治疗时间 " + getTimeString(m_totalTime.spendTime);
    ui->label_time_treatment->setText(str);
}

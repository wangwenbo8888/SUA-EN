...
void write_bmp_header(j_decompress_ptr cinfo, char* output_data)
{
    ...
    memcpy(output_data,&bmpfileheader[0],14);
    memcpy(output_data + 14, &bmpinfoheader[0], 40);
}
...
int read_jpeg_file(const char *input_filename,unsigned char*& output_buffer,int& size)
{
    ...
    size = 56 + row_width * cinfo.output_height;
    // output_buffer = new unsigned char[size];

    write_bmp_header(&cinfo, (char*)output_buffer);

    // output_buffer = (unsigned char *)malloc(row_width * cinfo.output_height);
    unsigned char* temp = output_buffer + 56;
    memset(temp, 0, row_width * cinfo.output_height);
    tmp = temp;

...
double GeoMatch::FindGeoMatchModel(const void* srcarr,double minScore,double greediness,CvPoint *resultPoint)
{
    ...
    int halfHeight = Ssize.height*0.5;
    // 超声图像只有上半部分肌瘤能看到，所以也只拿核磁肌瘤的上半部分去匹配
    for( i = 0; i < halfHeight/*Ssize.height*/; i++ )
    {
...
bool GeoMatch::GetBegin(const CvPoint& result,CvPoint& point)
{
    point.y=cordinates[0].x + result.x;
    point.x=cordinates[0].y + result.y;
    return true;
}

bool GeoMatch::GetStart(CvPoint& point)
{
    if (cordinates!=NULL)
    {
        point.x = cordinates[0].x + centerOfGravity.x;
        point.y = cordinates[0].y + centerOfGravity.y;
        return true;
    }
    else
    {

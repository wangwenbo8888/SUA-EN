...
#define		OK_AC5167				215067 // ccd color 5.0 M Pixels (double channels)
...
#define		DEVICE_SELECTCLCFGNUM	12 //select the number of code-config to use in camlink, if HIWORD(lParam)=0, LOWOR(lParam) 
										//is standard num(1~5);    if HIWORD(lParam)=1, LOWOR(lParam) is user-defined num(>0).
										//specially if LOWORD(lParam)=0, return total numbers can be defined in camlink

#define		DEVICE_SETCLCFGCODE		13 //set config-code to currently used in camlink, lParam is structure CLCFGPARAM pointer
#define		DEVICE_GETCLCFGCODE		14 //get config-code from currently used in camlink, lParam is structure CLCFGPARAM pointer
...
{
	WORD	wStructSize;		//struct size
	WORD	wNumChips;			//just return max num of code-chips(current is 1~3), 
